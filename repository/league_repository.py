...
class LeagueRepository:
    ...
    def get_league_config(self, country: str, division: str) -> LeagueConfig or None:
        with open(self._league_config_path(country, division), 'r', encoding='utf-8') as fp:
            league_config: LeagueConfig = json.load(fp) 
        return league_config

    def save_league_config(self, league_config: LeagueConfig) -> None:
        with open(self._league_config_path(league_config.country, league_config.name), 'r', encoding='utf-8') as fp:
            json.dump(league_config, fp)

    def league_exists(self, country: str, divivsion: str) -> bool:
    ...
    def load_league(self, league: League) -> Union[pd.DataFrame, None]:
        if self.league_exists(league.country, league.name):
    ...
    def save_league(self, 
        ...
        if self.league_exists(league.country, league.name) and update_on_exist is False:
            return None
        ...
        self.save_league_config(league_config)

    def delete_league(self, league: League) -> bool:
        if self.league_exists(league.country, league.name) is False:
            return False
